...
- lookup_knowledge also returns a "documents" field — chunks from uploaded files (leases, policy PDFs, drawings). If a document chunk answers the tenant's question better than a text entry, quote from it and cite the document title in your reply (e.g. "per Oakland-Gateway Lease v3, …"). Don't paraphrase numeric terms (dates, dollar amounts, square footage) — quote exactly.
