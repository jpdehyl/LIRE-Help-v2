...
function StageColumn({
  ...
  return (
    <div className="flex min-h-[260px] w-[260px] shrink-0 flex-col rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/60">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{stage.label}</h3>
        <span className="rounded-full bg-white px-2 py-0.5 text-xs font-medium text-slate-500 ring-1 ring-inset ring-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:ring-slate-700">
          {deals.length}
        </span>
      </div>
      <div className="mt-3 flex flex-1 flex-col gap-2">
        {deals.length === 0 ? (
          <p className="text-xs text-slate-400 dark:text-slate-500">No deals in this stage.</p>
        ) : (
          deals.map((deal) => {
            const unit = deal.unitId ? unitsById[deal.unitId] : null;
            return (
              <article key={deal.id} className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{deal.prospectCompany}</p>
                {deal.prospectContactName ? (
                  <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{deal.prospectContactName}</p>
                ) : null}
                <div className="mt-2 flex flex-wrap gap-1.5 text-[11px] font-medium text-slate-500 dark:text-slate-400">
                  {unit ? (
                    <span className="rounded-full bg-slate-50 px-2 py-0.5 ring-1 ring-inset ring-slate-200 dark:bg-slate-800 dark:ring-slate-700">
                      {unit.label}
                    </span>
                  ) : null}
                  {deal.sizeNeededSqFt ? (
                ...
                {deal.notes ? <p className="mt-2 text-xs text-slate-600 dark:text-slate-300">{deal.notes}</p> : null}
              </article>
            );
          })
        )}
    ...
    <section className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Unit inventory</p>
        ...
        <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
          ...
          <tbody className="divide-y divide-slate-100 bg-white dark:divide-slate-800 dark:bg-slate-900">
            {units.length === 0 ? (
              ...
              units.map((u) => (
                <tr key={u.id}>
                  <td className="px-4 py-2 font-medium text-slate-900 dark:text-slate-100">{u.label}</td>
                  <td className="px-4 py-2 text-slate-600 dark:text-slate-300">{u.sqFt?.toLocaleString() ?? "—"}</td>
                  <td className="px-4 py-2">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${
                        u.availability === "vacant"
                          ? "bg-emerald-50 text-emerald-700 ring-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:ring-emerald-900/60"
                          : u.availability === "pending"
                          ? "bg-amber-50 text-amber-700 ring-amber-200 dark:bg-amber-500/10 dark:text-amber-300 dark:ring-amber-900/60"
                  ...
                  <td className="px-4 py-2 text-slate-600 dark:text-slate-300">{u.askingRateUsd ?? "—"}</td>
                  <td className="px-4 py-2 text-slate-600 dark:text-slate-300">{u.notes ?? "—"}</td>
                </tr>
              ))
            )}
          </tbody>
      ...
      <div className="space-y-6">
        <section className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Deal pipeline</p>
              <h2 className="mt-1 text-lg font-semibold text-slate-950 dark:text-slate-50">
          ...
          {config ? (
            ...
            <p className="mt-6 text-sm text-slate-400 dark:text-slate-500">Loading pipeline config…</p>
          )}
        </section>

        <UnitInventory units={units} onSync={handleSync} />

        <section className="rounded-[24px] border border-dashed border-slate-300 bg-slate-50 p-6 text-sm leading-relaxed text-slate-600 dark:border-slate-700 dark:bg-slate-900/40 dark:text-slate-300">
